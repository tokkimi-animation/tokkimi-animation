...
CAST = [
    ("별이", "BYEORI", "byeori-pointing.png", (255, 213, 76), "spin"),
    ("몽이", "MONGI", "mongi-sneeze.png", (161, 220, 241), "sneeze"),
    ("콩콩", "KONGKONG", "kongkong-jump.png", (226, 161, 96), "jump"),
    ("토리", "TORI", "tori-inventing.png", (239, 151, 102), "invent"),
    ("밤밤", "BAMBAM", "bambam-brave.png", (203, 158, 112), "brave"),
    ("루미", "LUMI", "lumi-discovery.png", (180, 160, 221), "glide"),
    ("달할머니", "MOON GRANDMA", "moon-grandma-comfort.png", (239, 204, 147), "magic"),
]
...
def character_motion(path, action, duration, index):
    ...
    return clip.with_effects([vfx.FadeIn(0.12), vfx.FadeOut(0.12)])
...
def cast_scene(item, index, duration=1.65):
    name, latin, path, color, action = item
    right = index % 2 == 0
    return CompositeVideoClip([
        backdrop(index + 1, duration),
        character_motion(path, action, duration, index),
...
def luni_final(duration):
    ...
    position = lambda t: (355 + int(10*math.sin(t*3.2)), 92-int(18*math.sin(min(1,t/.65)*math.pi)))
    normal1 = (
        ImageClip(str(CHARACTERS / "luni.png"), duration=1.25)
        .resized(height=570).with_position(position)
    )
    wink1 = (
        ImageClip(str(POSES / "luni-wink.png"), duration=0.47)
        .resized(height=570).with_start(1.25).with_position(position)
    )
    normal2 = (
        ImageClip(str(CHARACTERS / "luni.png"), duration=0.28)
        .resized(height=570).with_start(1.72).with_position(position)
    )
    wink2 = (
        ImageClip(str(POSES / "luni-wink.png"), duration=duration-2.0)
        .resized(height=570).with_start(2.0).with_position(position)
    )
    image = Image.new("RGBA", SIZE, (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    ...
    draw.text(((1280-(box[2]-box[0]))/2, 580), phrase, font=FONT_NAME,
              fill=(79, 63, 137, 255))
    return CompositeVideoClip(
        [bg, normal1, wink1, normal2, wink2, ImageClip(np.array(image), duration=duration)],
        size=SIZE,
...
async def final_voice(path):
    await create_voice(
        {"speaker": "루니", "text": "나야, 루니!", "voice": "ko-KR-SunHiNeural"},
        path,
    )
...
def main():
    ...
    voice_path = WORK / "luni-final.mp3"
    if voice_path.exists():
        voice_path.unlink()
    asyncio.run(final_voice(voice_path))
    ...
    voice = AudioFileClip(str(voice_path)).with_volume_scaled(1.18)
    voice_start = video.duration - 2.55
    voice = voice.with_start(voice_start)
    video = video.with_audio(CompositeAudioClip([music_clip, voice]))

    output = OUTPUT / "LUNI-GENERIQUE-V2-CHAQUE-EPISODE.mp4"
    ...
    (OUTPUT / "LISEZ-MOI.txt").write_text(
        "Version recommandée : LUNI-GENERIQUE-V2-CHAQUE-EPISODE.mp4\n"
        "Présentation individuelle des personnages, musique mélodique, "
        "puis Luni : « 나야, 루니! » avec un clin d’œil.\n",
        encoding="utf-8",
    )
...
<body>
  <main>
    ...
    <p>Présentation animée des personnages, puis Luni dit « 나야, 루니! ».</p>
    <video controls autoplay preload="auto" poster="tokkimi-logo.png">
      <source src="LUNI-GENERIQUE-V2-CHAQUE-EPISODE.mp4" type="video/mp4">
    </video>
    <nav>
      <a href="LUNI-GENERIQUE-V2-CHAQUE-EPISODE.mp4" download>
